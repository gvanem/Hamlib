...
int main(int argc, char *argv[])
{
    ...
    if (rot_file)
    {
        strncpy(HAMLIB_ROTPORT(my_rot)->pathname, rot_file, HAMLIB_FILPATHLEN - 1);
    }

    if (rot_file2)
    {
        strncpy(HAMLIB_ROTPORT2(my_rot)->pathname, rot_file2, HAMLIB_FILPATHLEN - 1);
    }

    /* FIXME: bound checking and port type == serial */
    if (serial_rate != 0)
    {
        HAMLIB_ROTPORT(my_rot)->parm.serial.rate = serial_rate;
    }
