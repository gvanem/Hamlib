...
int main(int argc, char *argv[])
{
    ...
    if (amp_file)
    {
        strncpy(HAMLIB_AMPPORT(my_amp)->pathname, amp_file, HAMLIB_FILPATHLEN - 1);
        strncpy(HAMLIB_AMPSTATE(my_amp)->ampport_deprecated.pathname, amp_file,
                HAMLIB_FILPATHLEN - 1);
    }
    ...
    if (serial_rate != 0)
    {
        HAMLIB_AMPPORT(my_amp)->parm.serial.rate = serial_rate;
        HAMLIB_AMPSTATE(my_amp)->ampport_deprecated.parm.serial.rate = serial_rate;
    }
