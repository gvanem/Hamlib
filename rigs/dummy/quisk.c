/*
 *  Hamlib Quisk backend - main file
 ...
 */
static int quisk_vfostr(RIG *rig, char *vfostr, int len, vfo_t vfo)
{
    struct quisk_priv_data *priv;
...
static int quisk_open(RIG *rig)
{
    ...
    ret = quisk_transaction(rig, cmd, strlen(cmd), buf);
...
static int quisk_set_freq(RIG *rig, vfo_t vfo, freq_t freq)
{
    int ret;
    char cmd[CMD_MAX];
    char buf[BUF_MAX];
    ...
    ret = quisk_vfostr(rig, vfostr, sizeof(vfostr), vfo);
...
static int quisk_get_freq(RIG *rig, vfo_t vfo, freq_t *freq)
{
    int ret;
    char cmd[CMD_MAX];
    char buf[BUF_MAX];
...
#if 0
static int quisk_set_vfo(RIG *rig, vfo_t vfo)
{
    int ret;
    char cmd[CMD_MAX];
    char buf[BUF_MAX];
...
static int quisk_get_vfo(RIG *rig, vfo_t *vfo)
{
    int ret;
    char cmd[CMD_MAX];
    char buf[BUF_MAX];
...
static int quisk_set_ptt(RIG *rig, vfo_t vfo, ptt_t ptt)
{
    int ret;
    char cmd[CMD_MAX];
    char buf[BUF_MAX];
...
static int quisk_get_ptt(RIG *rig, vfo_t vfo, ptt_t *ptt)
{
    int ret;
    char cmd[CMD_MAX];
    char buf[BUF_MAX];
...
static int quisk_get_dcd(RIG *rig, vfo_t vfo, dcd_t *dcd)
{
    int ret;
    char cmd[CMD_MAX];
    char buf[BUF_MAX];
...
static int quisk_set_rptr_offs(RIG *rig, vfo_t vfo, shortfreq_t rptr_offs)
{
    int ret;
    char cmd[CMD_MAX];
    char buf[BUF_MAX];
...
static int quisk_get_rptr_offs(RIG *rig, vfo_t vfo, shortfreq_t *rptr_offs)
{
    int ret;
    char cmd[CMD_MAX];
    char buf[BUF_MAX];
...
static int quisk_set_ctcss_tone(RIG *rig, vfo_t vfo, tone_t tone)
{
    int ret;
    char cmd[CMD_MAX];
    char buf[BUF_MAX];
...
static int quisk_get_ctcss_tone(RIG *rig, vfo_t vfo, tone_t *tone)
{
    int ret;
    char cmd[CMD_MAX];
    char buf[BUF_MAX];
...
static int quisk_set_dcs_code(RIG *rig, vfo_t vfo, tone_t code)
{
    int ret;
    char cmd[CMD_MAX];
    char buf[BUF_MAX];
...
static int quisk_get_dcs_code(RIG *rig, vfo_t vfo, tone_t *code)
{
    int ret;
    char cmd[CMD_MAX];
    char buf[BUF_MAX];
...
static int quisk_set_ctcss_sql(RIG *rig, vfo_t vfo, tone_t tone)
{
    int ret;
    char cmd[CMD_MAX];
    char buf[BUF_MAX];
...
static int quisk_get_ctcss_sql(RIG *rig, vfo_t vfo, tone_t *tone)
{
    int ret;
    char cmd[CMD_MAX];
    char buf[BUF_MAX];
...
static int quisk_set_dcs_sql(RIG *rig, vfo_t vfo, unsigned int code)
{
    int ret;
    char cmd[CMD_MAX];
    char buf[BUF_MAX];
...
static int quisk_get_dcs_sql(RIG *rig, vfo_t vfo, unsigned int *code)
{
    int ret;
    char cmd[CMD_MAX];
    char buf[BUF_MAX];
...
#if 0
static int quisk_set_split_freq(RIG *rig, vfo_t vfo, freq_t tx_freq)
{
    int ret;
    char cmd[CMD_MAX];
    char buf[BUF_MAX];
...
static int quisk_get_split_freq(RIG *rig, vfo_t vfo, freq_t *tx_freq)
{
    int ret;
    char cmd[CMD_MAX];
    char buf[BUF_MAX];
...
static int quisk_set_rit(RIG *rig, vfo_t vfo, shortfreq_t rit)
{
    int ret;
    char cmd[CMD_MAX];
    char buf[BUF_MAX];
...
static int quisk_get_rit(RIG *rig, vfo_t vfo, shortfreq_t *rit)
{
    int ret;
    char cmd[CMD_MAX];
    char buf[BUF_MAX];
...
#if 0
static int quisk_set_xit(RIG *rig, vfo_t vfo, shortfreq_t xit)
{
    int ret;
    char cmd[CMD_MAX];
    char buf[BUF_MAX];
...
static int quisk_get_xit(RIG *rig, vfo_t vfo, shortfreq_t *xit)
{
    int ret;
    char cmd[CMD_MAX];
    char buf[BUF_MAX];
...
#if 0
static int quisk_set_ts(RIG *rig, vfo_t vfo, shortfreq_t ts)
{
    int ret;
    char cmd[CMD_MAX];
    char buf[BUF_MAX];
...
static int quisk_get_ts(RIG *rig, vfo_t vfo, shortfreq_t *ts)
{
    int ret;
    char cmd[CMD_MAX];
    char buf[BUF_MAX];
...
static int quisk_set_func(RIG *rig, vfo_t vfo, setting_t func, int status)
{
    int ret;
    char cmd[CMD_MAX];
    char buf[BUF_MAX];
...
static int quisk_get_func(RIG *rig, vfo_t vfo, setting_t func, int *status)
{
    int ret;
    char cmd[CMD_MAX];
    char buf[BUF_MAX];
...
#if 0
static int quisk_set_powerstat(RIG *rig, powerstat_t status)
{
    int ret;
    char cmd[CMD_MAX];
    char buf[BUF_MAX];
...
static int quisk_get_powerstat(RIG *rig, powerstat_t *status)
{
    int ret;
    char cmd[CMD_MAX];
    char buf[BUF_MAX];
...
static int quisk_set_parm(RIG *rig, setting_t parm, value_t val)
{
    int ret;
    char cmd[CMD_MAX];
    char buf[BUF_MAX];
...
static int quisk_get_parm(RIG *rig, setting_t parm, value_t *val)
{
    int ret;
    char cmd[CMD_MAX];
    char buf[BUF_MAX];
...
#if 0
static int quisk_set_ant(RIG *rig, vfo_t vfo, ant_t ant, value_t option)
{
    int ret;
    char cmd[CMD_MAX];
    char buf[BUF_MAX];
...
static int quisk_set_bank(RIG *rig, vfo_t vfo, int bank)
{
    int ret;
    char cmd[CMD_MAX];
    char buf[BUF_MAX];
...
static int quisk_set_mem(RIG *rig, vfo_t vfo, int ch)
{
    int ret;
    char cmd[CMD_MAX];
    char buf[BUF_MAX];
...
static int quisk_get_mem(RIG *rig, vfo_t vfo, int *ch)
{
    int ret;
    char cmd[CMD_MAX];
    char buf[BUF_MAX];
...
static int quisk_scan(RIG *rig, vfo_t vfo, scan_t scan, int ch)
{
    int ret;
    char cmd[CMD_MAX];
    char buf[BUF_MAX];
...
#if 0
static int quisk_vfo_op(RIG *rig, vfo_t vfo, vfo_op_t op)
{
    int ret;
    char cmd[CMD_MAX];
    char buf[BUF_MAX];
...
static const char *quisk_get_info(RIG *rig)
{
    int ret;
    char cmd[CMD_MAX];
...
#if 0
static int quisk_send_dtmf(RIG *rig, vfo_t vfo, const char *digits)
{
    int ret, len;
    ...
    ret = quisk_transaction(rig, cmdp, strlen(cmdp), buf);
    free(cmdp);
...
#if 0
static int quisk_recv_dtmf(RIG *rig, vfo_t vfo, char *digits, int *length)
{
    int ret;
    char cmd[CMD_MAX];
    static char buf[BUF_MAX];
...
#if 0
static int quisk_send_voice_mem(RIG *rig, vfo_t vfo, int ch)
{
    int ret;
    char cmd[CMD_MAX];
    char buf[BUF_MAX];
...
#if 0
static int quisk_send_morse(RIG *rig, vfo_t vfo, const char *msg)
{
    int ret, len;
...
static int quisk_stop_morse(RIG *rig, vfo_t vfo)
{
    int ret;
    char cmd[] = "\\stop_morse\n";
    char buf[BUF_MAX];
...
static int quisk_set_vfo_opt(RIG *rig, int status)
{
    char cmdbuf[32];
    char buf[BUF_MAX];
    int ret;
    ...
    ret = quisk_transaction(rig, cmdbuf, strlen(cmdbuf), buf);
...
static int quisk_set_trn(RIG *rig, int trn)
{
    char cmdbuf[32];
    char buf[BUF_MAX];
    int ret;
...
static int quisk_get_trn(RIG *rig, int *trn)
{
    char cmdbuf[32];
    char buf[BUF_MAX];
    int ret;
...
int quisk_password(RIG *rig, const char *key1)
{
    char cmdbuf[256];
    char buf[BUF_MAX];
    int retval;
...
struct rig_caps quisk_caps =
{
    RIG_MODEL(RIG_MODEL_QUISK),
    .model_name =     "Quisk",
    .mfg_name =       "N2ADR James Ahlstrom",
    .version =        "20230709.0",
    .copyright =      "LGPL",
    ...
    .rig_init =     quisk_init,
    .rig_cleanup =  quisk_cleanup,
    .rig_open =     quisk_open,
    .rig_close =    quisk_close,

    .set_freq =     quisk_set_freq,
    .get_freq =     quisk_get_freq,
    .set_mode =     quisk_set_mode,
    .get_mode =     quisk_get_mode,
//    .set_vfo =      quisk_set_vfo,
//    .get_vfo =      quisk_get_vfo,

//    .set_powerstat =  quisk_set_powerstat,
//    .get_powerstat =  quisk_get_powerstat,
    .set_level =     quisk_set_level,
    .get_level =     quisk_get_level,
    .set_func =      quisk_set_func,
    .get_func =      quisk_get_func,
    .set_parm =      quisk_set_parm,
    .get_parm =      quisk_get_parm,

    .get_info =      quisk_get_info,


    .set_ptt =    quisk_set_ptt,
    .get_ptt =    quisk_get_ptt,
    .get_dcd =    quisk_get_dcd,
    .set_rptr_shift =     quisk_set_rptr_shift,
    .get_rptr_shift =     quisk_get_rptr_shift,
    .set_rptr_offs =  quisk_set_rptr_offs,
    .get_rptr_offs =  quisk_get_rptr_offs,
    .set_ctcss_tone =     quisk_set_ctcss_tone,
    .get_ctcss_tone =     quisk_get_ctcss_tone,
    .set_dcs_code =   quisk_set_dcs_code,
    .get_dcs_code =   quisk_get_dcs_code,
    .set_ctcss_sql =  quisk_set_ctcss_sql,
    .get_ctcss_sql =  quisk_get_ctcss_sql,
    .set_dcs_sql =    quisk_set_dcs_sql,
    .get_dcs_sql =    quisk_get_dcs_sql,
//    .set_split_freq =     quisk_set_split_freq,
//    .get_split_freq =     quisk_get_split_freq,
//    .set_split_mode =     quisk_set_split_mode,
//    .get_split_mode =     quisk_get_split_mode,
//    .set_split_vfo =  quisk_set_split_vfo,
//    .get_split_vfo =  quisk_get_split_vfo,
    .set_rit =    quisk_set_rit,
    .get_rit =    quisk_get_rit,
//    .set_xit =    quisk_set_xit,
//    .get_xit =    quisk_get_xit,
//    .set_ts =     quisk_set_ts,
//    .get_ts =     quisk_get_ts,
//    .set_ant =    quisk_set_ant,
//    .get_ant =    quisk_get_ant,
//    .set_bank =   quisk_set_bank,
//    .set_mem =    quisk_set_mem,
//    .get_mem =    quisk_get_mem,
//    .vfo_op =     quisk_vfo_op,
//    .scan =       quisk_scan,
//    .send_dtmf =  quisk_send_dtmf,
//    .recv_dtmf =  quisk_recv_dtmf,
//    .send_morse =  quisk_send_morse,
//    .send_voice_mem =  quisk_send_voice_mem,
//    .stop_morse =  quisk_stop_morse,
//    .set_channel =    quisk_set_channel,
//    .get_channel =    quisk_get_channel,
    .set_vfo_opt = quisk_set_vfo_opt,
    //.set_trn =    quisk_set_trn,
    //.get_trn =    quisk_get_trn,
    .power2mW =   quisk_power2mW,
    .mW2power =   quisk_mW2power,
    .password =   quisk_password,
//    .set_lock_mode = quisk_set_lock_mode,
//    .get_lock_mode = quisk_get_lock_mode,
