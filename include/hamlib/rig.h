...
extern HAMLIB_EXPORT_VAR(int) skip_init;   // allow rigctl to skip any radio commands at startup
...
extern HAMLIB_EXPORT(void)
rig_set_debug_file_line HAMLIB_PARAMS((int flag));
...
#define ELAPSED2 rig_debug (RIG_DEBUG_VERBOSE, "%s%d:%s: elapsed=%.0lfms\n", \
                            hl_stars(HAMLIB_STATE(rig)->depth),              \
                            HAMLIB_STATE(rig)->depth, __func__,              \
                            elapsed_ms(&__begin, HAMLIB_ELAPSED_GET))
...
extern HAMLIB_EXPORT(const char *)
hl_stars HAMLIB_PARAMS((int len));
...
extern HAMLIB_EXPORT_VAR(powerstat_t) rig_powerstat; // for use by both rigctld and rigctl
extern HAMLIB_EXPORT_VAR(int)         lock_mode;
